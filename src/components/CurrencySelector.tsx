...
export default function CurrencySelector({
  ...
  // Compact variant — flag only, dropdown shows full labels
  if (variant === "compact") {
    if (isLocked) {
      return (
        <div ref={ref} className="relative">
          <button
            onClick={handleLockedClick}
            className="px-1.5 py-1 bg-white text-navy text-sm rounded-md transition-colors flex items-center gap-0.5"
          >
            <span>{currentFlag}</span>
            <svg
              className="w-3 h-3 text-navy/40"
    ...
    return (
      <div ref={ref} className="relative">
        <button
          ...
          className="px-2 py-1.5 bg-white text-navy hover:bg-white/90 text-sm rounded-md transition-colors flex items-center gap-1"
        >
          <span>{currentFlag}</span>
          <svg
