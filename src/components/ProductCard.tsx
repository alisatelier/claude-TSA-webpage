...
function getAllImages(product: Product): string[] {
  ...
  if (product.variationImages) {
    // Collect images from ALL variations (skip Imperfect, empty arrays and duplicates)
    Object.entries(product.variationImages).forEach(([key, varImages]) => {
      // Skip Imperfect images
...
function ProductCardCarousel({ images, alt, syncIndex }: { images: string[]; alt: string; syncIndex?: number }) {
  const [internalIndex, setInternalIndex] = useState(0);
  const [loadedImages, setLoadedImages] = useState<Set<number>>(new Set([0]));
  const [isAutoScrolling, setIsAutoScrolling] = useState(false);

  // Use syncIndex if provided, otherwise use internal state
  const activeIndex = syncIndex !== undefined ? syncIndex % images.length : internalIndex;
