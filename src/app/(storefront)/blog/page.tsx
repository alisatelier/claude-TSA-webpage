          ...
          <div className="max-w-5xl mx-auto">
            <div className="bg-gradient-to-br from-cream to-light-blush rounded-xl overflow-hidden">
              <div className="grid grid-cols-1 md:grid-cols-2">
                <div className="aspect-[4/3] bg-gradient-to-br from-navy/5 to-blush/20 flex items-center justify-center relative">
                  {featured.image ? (
                    <Image
                      src={featured.image}
                      alt={featured.title}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {rest.map((post) => (
              <article key={post.id} className="bg-white rounded-xl overflow-hidden shadow-[0_4px_12px_rgba(83,91,115,0.08)] hover:shadow-[0_8px_24px_rgba(83,91,115,0.15)] transition-all duration-300">
                <div className="aspect-[16/10] bg-gradient-to-br from-cream to-light-blush flex items-center justify-center relative">
                  {post.image ? (
                    <Image
                      src={post.image}
                      alt={post.title}
                ...
                <div className="p-6">
                  ...
                  <Link href={`/blog/${post.slug}`} className="text-navy font-medium text-sm tracking-wider uppercase hover:text-mauve transition-colors inline-flex items-center gap-2">
                    Read More
                    <FontAwesomeIcon icon={faArrowRight} className="w-4 h-4" />
                  </Link>
                </div>
