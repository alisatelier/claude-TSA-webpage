...
export default function ProductPage() {
  ...
  const currentPrice = getProductPrice(product.prices);
  const displayPrice = isImperfect ? Math.round(currentPrice * 0.6) : currentPrice;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mb-6">
                {isImperfect ? (
                  <>
                    <p className="text-3xl font-semibold text-navy">{formatPrice(displayPrice)}</p>
                    <p className="text-xl text-mauve/60 line-through">{formatPrice(currentPrice)}</p>
                    <span className="px-2 py-1 bg-blush/40 text-navy text-xs font-medium rounded-full">40% OFF</span>
                  </>
                ) : (
                  <p className="text-3xl font-semibold text-navy">{formatPrice(currentPrice)}</p>
                )}
              </div>
